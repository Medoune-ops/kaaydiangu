...
import { coursSchema } from "@/lib/validations";
...
export async function GET(req: NextRequest) {
  try {
    ...
    if (session.user.role === "PROFESSEUR") {
      const where = { depose_par_id: session.user.id };
      const includeOpts = {
      ...
      if (page) {
        ...
        const [data, total] = await Promise.all([
          prisma.cours.findMany({ where, include: includeOpts, orderBy: { date: "desc" }, skip, take: limit }),
          prisma.cours.count({ where }),
        ]);
        return NextResponse.json({ data, pagination: { page: pageNum, limit, total, totalPages: Math.ceil(total / limit) } });
      }

      const cours = await prisma.cours.findMany({ where, include: includeOpts, orderBy: { date: "desc" } });
    ...
    if (session.user.role === "ELEVE") {
      ...
      const where = { classe_id: eleve.classe_id };
      const includeOpts = {
        matiere: { select: { nom: true } },
        classe: { select: { nom: true } },
    ...
    console.error("[COURS_GET] Erreur:", error);
...
export async function POST(req: NextRequest) {
  try {
    ...
    const parsed = coursSchema.safeParse(body);
    if (!parsed.success) {
      return NextResponse.json(
    ...
    const { titre, description, fichier_url, matiere_id, classe_id } = parsed.data;
    ...
    console.error("[COURS_POST] Erreur:", error);
    return NextResponse.json(
      { error: "Erreur serveur interne" },
      { status: 500 }
    );
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    console.error("[COURS_DELETE] Erreur:", error);
    return NextResponse.json(
      { error: "Erreur serveur interne" },
      { status: 500 }
    );
